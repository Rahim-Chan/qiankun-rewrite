  ...
  const loadA = async() =>{
    if (app1) {
      ...
      setApp1(loadMicroApp(
        // { name: 'react16', entry: 'http://localhost:3001/app-tem/', container: '#react16' }
        { name: 'react16', entry: {
          html: '<div id="app"></div>',
          scripts: ['http://localhost:3001/app-tem/static/js/MessageCenter-app-tem.js'],
        }, container: '#react16' }
  ...
  const loadB = async() =>{
    if (app2) {
      ...
      setApp2(loadMicroApp(
        { name: 'react161', entry: '//localhost:10086', container: '#react161' },
      ))
    }
  }
