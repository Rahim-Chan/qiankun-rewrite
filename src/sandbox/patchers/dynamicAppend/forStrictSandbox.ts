...
import {
  ...
  rawHTMLBodyAppendChild,
  calcAppCount,
  getAppWrapperHeadElement,
  isAllAppsUnmounted,
  isHijackingTag,
  patchHTMLDynamicAppendPrototypeFunctions,
  rawHTMLHeadAppendChild,
  rebuildCSSRules,
    ...
    return function rebuild() {
      rebuildCSSRules(dynamicStyleSheetElements, (stylesheetElement) => {
        ...
        if (!appWrapper.contains(stylesheetElement)) {
          const isHead = stylesheetElement[styleElementTargetSymbol] === 'head';
          const mountDom = isHead ? getAppWrapperHeadElement(appWrapper) : getAppWrapperBodyElement(appWrapper);
          (isHead ? rawHTMLHeadAppendChild : rawHTMLBodyAppendChild).call(mountDom, stylesheetElement);
          return true;
        }
