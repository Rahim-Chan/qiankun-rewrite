...
const patchMethodsMap = new Map();
export const rawHTMLHeadAppendChild = HTMLHeadElement.prototype.appendChild;
export const rawHTMLBodyAppendChild = HTMLBodyElement.prototype.appendChild;
...
export const getAppWrapperBodyElement = (appWrapper: Element | ShadowRoot): Element => {
  return appWrapper.querySelector(qiankunBodyTagName)!;
};
    ...
    if (element.tagName) {
      ...
      switch (element.tagName) {
        ...
        case STYLE_TAG_NAME: {
          ...
          const mountDOM =
            target === 'head' ? getAppWrapperHeadElement(appWrapper) : getAppWrapperBodyElement(appWrapper);
...
function getNewRemoveChild(
  ...
  return function removeChild<T extends Node>(this: HTMLHeadElement | HTMLBodyElement, child: T) {
    ...
    const config = containerConfigGetter(child as any);
    if (!isHijackingTag(tagName) || !config) {
      return headOrBodyRemoveChild.call(this, child) as T;
    }

    try {
      let attachedElement: Node;
      const { appWrapperGetter, dynamicStyleSheetElements } = config;
      ...
      const appWrapper = appWrapperGetter();
      const container = target === 'head' ? getAppWrapperHeadElement(appWrapper) : getAppWrapperBodyElement(appWrapper);
...
export function patchHTMLDynamicAppendPrototypeFunctions(
  ...
  const beforeHTMLHeadElementAppendChild = patchMethodsMap.get(HTMLHeadElement.prototype.appendChild);
  const beforeHTMLBodyElementAppendChild = patchMethodsMap.get(HTMLBodyElement.prototype.appendChild);
  const beforeHTMLHeadElementInsertBefore = patchMethodsMap.get(HTMLHeadElement.prototype.insertBefore);

  if (!beforeHTMLHeadElementAppendChild && !beforeHTMLBodyElementAppendChild && !beforeHTMLHeadElementInsertBefore) {
    ...
    patchMethodsMap.set(HTMLHeadElement.prototype.appendChild, rawHeadAppendChild);
    patchMethodsMap.set(HTMLBodyElement.prototype.appendChild, rawBodyAppendChild);
    patchMethodsMap.set(HTMLHeadElement.prototype.insertBefore, rawHeadInsertBefore);
  }
  ...
  const beforeHTMLHeadElementRemoveChild = patchMethodsMap.get(HTMLHeadElement.prototype.removeChild);
  const beforeHTMLBodyElementRemoveChild = patchMethodsMap.get(HTMLBodyElement.prototype.removeChild);

  // Just overwrite it while it have not been overwrite
  if (!beforeHTMLHeadElementRemoveChild && !beforeHTMLBodyElementRemoveChild) {
    ...
    patchMethodsMap.set(HTMLHeadElement.prototype.removeChild, beforeHTMLHeadElementRemoveChild);
    patchMethodsMap.set(HTMLBodyElement.prototype.removeChild, beforeHTMLBodyElementRemoveChild);
  }
